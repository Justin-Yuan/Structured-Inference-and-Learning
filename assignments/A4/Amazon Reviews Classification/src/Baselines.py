...
class MajorityClass(Classifier):
    ...
    def evaluate_majority_predictor(self, y_test_data=None):
        ...
        if y_test_data == None:
            ...
            # preds[:, self.model] = 1 
            acc = np.mean(1*np.equal(np.argmax(np.array(self.y_test), 1), self.model))
        else:
            pred = self.model * np.ones(shape=(y_test_data.shape[0], y_test_data.shape[1]))
            acc = np.mean(1*np.equal(np.argmax(np.array(self.y_test_data), 1), self.model))
    ...
    def load_model(self):
        """ load the model, just a label number
        """
        with open('../models/majority_class.pkl', 'rb') as f:
            self.model = pickle.load(f)
    


class LogisticRegression(Classifier):
    """ multinomial logistic regression using word indices as features 
    """
    ...
    def build_logistic_regression(self):
        """
        """
        sequence_input = Input(shape=(self.max_sequence_length, ), dtype='float32')
        preds = Dense(self.num_labels, activation='softmax')(sequence_input)
...
class SVM(Classifier):
    ...
    def save_model(self, path='../models/svm/svm.pkl'):
        """ save the trained SVM classifier
        """
        joblib.dump(self.model, path) 

    def load_model(self, path='../models/svm/svm.pkl'):
...
if __name__ == '__main__':
    ...
    print("majority class preditor evaluated")
    majority_classifier.save_model()
    print("majority model saved", end='\n\n')
    ...
    logistic_classifier = LogisticRegression(batch_size=128, epochs=30, raw_data_path=None, embedded_data_path='../data/data_and_embedding100.npz')
